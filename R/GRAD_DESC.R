...
GD<- function(XX,Xy,b=rep(0,ncol(XX)),active=1:ncol(XX), RSS=1,nIter=10,learning_rate=mean(diag(XX))/10,lambda=0,b0=rep(0,ncol(XX)),lambda0=1){
    ...
    active <- active - 1L # for the 0-based index
        ans <- .Call("GRAD_DESC", s   XX, Xy, b, active, nIter, learning_rate)
    return(ans[[1]])
}
